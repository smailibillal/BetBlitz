...
CREATE VIEW statistiques_paris AS
SELECT 
    u.id,
    u.username,
    COUNT(CASE WHEN p.statut = 'EN_ATTENTE' THEN 1 END) as paris_en_cours,
    COUNT(CASE WHEN p.statut = 'GAGNE' THEN 1 END) as paris_gagnes,
    COUNT(CASE WHEN p.statut = 'PERDU' THEN 1 END) as paris_perdus,
    SUM(CASE WHEN p.statut = 'GAGNE' THEN p.montant * p.cote - p.montant ELSE 0 END) as gains_totaux,
    SUM(p.montant) as mises_totales,
    (COUNT(CASE WHEN p.statut = 'GAGNE' THEN 1 END) * 100.0 / 
        NULLIF(COUNT(CASE WHEN p.statut IN ('GAGNE', 'PERDU') THEN 1 END), 0)) as taux_reussite
FROM utilisateur u
